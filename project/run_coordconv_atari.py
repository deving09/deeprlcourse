...
def build_arg_parser():
    ...
    parser.add_argument("--model", default="atari", choices=["visual", "atari", "frozen", "finetune", "coordconv"])
...
def coord_conv_model(img_in, num_actions, scope, reuse=False):
    with tf.variable_scope(scope, reuse=reuse):
        out = img_in
        with tf.variable_scope("coordconvnet"):
            # original architecture
            x = tf.range(160, delta=1, dtype=tf.float32, name='x-coord')
            y = tf.range(210, delta=1, dtype=tf.float32, name='y-coord')
            X, Y = tf.meshgrid(x, y)
            X = tf.expand_dims(X, 2)
            Y = tf.expand_dims(Y, 2)
            batch_size = tf.shape(img_in)[0]
            X = tf.reshape(X, [batch_size, 210, 160, 1])
            Y = tf.reshape(Y, [batch_size, 210, 160, 1])
            out = tf.concat([out, X, Y], 3)
...
def main():
    ...
    if args.model == "coordconv":
        model = coord_conv_model
